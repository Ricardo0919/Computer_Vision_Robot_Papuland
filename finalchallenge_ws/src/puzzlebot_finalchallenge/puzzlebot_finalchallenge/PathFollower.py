#!/usr/bin/env python3
# ------------------------------------------------------------
# Puzzlebot Final Challenge – PathFollower (auto-reset odom)
# ------------------------------------------------------------
...
class PathFollower(Node):
    def __init__(self):
        ...
        self.active_path=[]; self.idx=0
        self.goal_th=0.08;   self.done=True
    ...
    # ---------- control ----------
    def control(self):
        if self.done or not self.active_path: return
        ...
        dt=(now-self.prev_t).nanoseconds*1e-9; self.prev_t=now

        d_ang=(err-self.prev_ang)/dt if dt else 0; self.prev_ang=err
